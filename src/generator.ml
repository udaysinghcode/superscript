...
let generate_js_func fname =
  let helper fname = match fname with
    ...
    | "__sub"     -> ("'function(a1) { return __box(\\'int\\', arguments.length === 0 ? 0 : arguments.length === 1 ? -1 * __unbox(a1) : Array.prototype.slice.call(arguments).map(__unbox).reduce(function(a,b){return a-b;})); }'", ["int"; "int"], "int", [])
    ...
    | "__subf"    -> ("'function(a1, a2) { return __box(\\'float\\', arguments.length === 0 ? 0 : arguments.length === 1 ? -1 * __unbox(a1) : Array.prototype.slice.call(arguments).map(__unbox).reduce(function(a,b){return a-b;})); }'", ["float"; "float"], "float", [])
