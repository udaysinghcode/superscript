...
(= filter (fn (f l)
  (fold_right 
    (fn (x y) (if (f x) (cons x y) y)) 
    l '())));;

(= partition (fn (f l)
  (fold_right 
    (fn (x y) 
      '((if (f x) (cons x (first y)) (first y)) 
        (if (f x) (second y) (cons x (second y))))) 
    l '('() '()))));;
...
(= zip (fn (a b)
  (zipwith (fn (x y) (cons x (cons y '()))) 
    a b)));;

(= zip3 (fn (a b c)
  (zipwith (fn (x y z) (cons x (cons y (cons z '()))))
    a b c)));;
